...
export class JourneyService {
	...
	async generateSitemap(): Promise<Buffer> {
		const smStream = new SitemapStream({ hostname: 'https://incisi.dev.br' });
		const pipeline = smStream.pipe(createGzip());

		smStream.write({ url: '/', changefreq: 'daily', priority: 1.0 });
		smStream.write({ url: '/journey', changefreq: 'weekly', priority: 0.8 });

		const articles = await this.findAllArticles();
		articles.forEach(article => {
		   smStream.write({ url: `/articles/${article.id}`, changefreq: 'monthly' });
		});
